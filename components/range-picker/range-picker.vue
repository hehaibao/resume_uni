			...
			<view class="r-dtpicker" :class="{'r-dtpicker-show':showPicker}">
				...
				<view class="rangeBox" @touchmove.stop.prevent="returnHandle" @tap.stop="returnHandle">
					<input type="text" disabled placeholder="开始时间" :value="startDate"
						:style="{color:themeColor,'border-color':themeColor,opacity:dateType=='startDate'?1:.5}"
						@tap="changeDateType('startDate')">
					至
					<input type="text" disabled placeholder="结束时间"
						:style="{color:themeColor,'border-color':themeColor,opacity:dateType=='endDate'?1:.5}"
	...
	export default {
		...
		methods: {
			...
			init() {
				var that = this,
					pickerValue = "";
				if ((this.fields == 'year' && this.start.length != 4) || (this.fields == 'month' && this.start.length !=
					7) || (this.fields == 'day' && this.start.length != 10)) {
					console.error("最小值格式与粒度格式不符");
					return;
				} else if ((this.fields == 'year' && this.end.length != 4) || (this.fields == 'month' && this.end.length !=
						7) || (this.fields == 'day' && this.end.length != 10)) {
					console.error("最大值格式与粒度格式不符");
					return;
				}
				var start = this.fields == 'year' ? this.start.slice(0, 4) : this.fields == 'month' ? this.start.slice(0,
						7) : this.start,
					end = this.fields == 'year' ? this.end.slice(0, 4) : this.fields == 'month' ? this.end.slice(0, 7) :
					this.end;
				if (!start || !end) {
				...
				if (this.value[0]) {
					if ((this.fields == 'year' && this.value[0].length != 4) || (this.fields == 'month' && this.value[0]
							.length != 7) || (this.fields == 'day' && this.value[0].length != 10)) {
						console.error("默认值格式与粒度格式不符");
						return;
					}
					this.startDate = this.value[0];
					if (this.value[1]) {
						if ((this.fields == 'year' && this.value[1].length != 4) || (this.fields == 'month' && this.value[
								1].length != 7) || (this.fields == 'day' && this.value[1].length != 10)) {
							console.error("默认值格式与粒度格式不符");
							return;
						}
						this.dateType = "startDate";
						this.endDate = this.value[1];
						if (this.fields == 'day') this.dayArr = this.getMonthDay(this.value[1].slice(0, 4), this.value[1]
							.slice(5, 7));
						pickerValue = this.getIndex(this.value[0]);
					} else {
						this.dateType = "startDate";
						if (this.fields == 'day') this.dayArr = this.getMonthDay(this.value[0].slice(0, 4), this.value[0]
							.slice(5, 7));
						pickerValue = this.getIndex(this.value[0]);
					}
				} else {
			...
			changeDateType(dateType) {
				...
				if (this[dateType]) {
					...
					var dateTxt = this.fields == 'year' ? this.yearArr[this.pickerValue[0]] : this.fields == 'month' ? this
						.yearArr[this.pickerValue[0]] + '-' + this.monthArr[this.pickerValue[1]] : this.yearArr[this
							.pickerValue[0]] + '-' + this.monthArr[this.pickerValue[1]] + '-' + this.dayArr[this
							.pickerValue[2]];
					this[dateType] = dateTxt;
					this.pickerValue = this.fields == 'year' ? [this.pickerValue[0]] : this.fields == 'month' ? [this
						.pickerValue[0], this.pickerValue[1]
			...
			pickerChangeMul(e) {
				...
				this[this.dateType] = this.fields == 'year' ? dateTxt.slice(0, 4) : this.fields == 'month' ? dateTxt.slice(
					0, 7) : dateTxt;
